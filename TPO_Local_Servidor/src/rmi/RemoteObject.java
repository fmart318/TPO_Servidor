...
public class RemoteObject extends UnicastRemoteObject implements RemoteInterface {
	...
	public RutaDTO obtenerMejorRuta(SucursalDTO origen, SucursalDTO destino) {
		...
		for (RutaDTO r : mp.getRutas()) {
			...
			if (r.getOrigen().getIdSucursal() == origen.getIdSucursal()
					&& r.getDestino().getIdSucursal() == destino.getIdSucursal()) {
	...
	public void enviar() {
		...
		// ORDERNAR PEDIDOS por prioridad y si la carga no esta despacha
		// entonces es que no llego al destino final
	...
	// Agregue cosas para que trate de enviar los pedidos prioritarios aunque no
	// llegue al 70%
